  ...
  list.forEach((p) => {
    ...
    card.innerHTML = `
      ...
      <div class="size-picker" role="group" aria-label="Choose size for ${escapeHtml(p.name)}">
        <button type="button" class="size-opt" data-ml="100">100ml<b>${money(p.prices[100])}</b></button>
        <button type="button" class="size-opt" data-ml="10">10ml<b>${money(p.prices[10])}</b></button>
      </div>
